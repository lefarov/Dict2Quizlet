...
def create_doc(name, folder_id, creds):
    service = build("drive", "v3", credentials=creds)
    file_metadata = dict(
        name=name, parents=[folder_id], mimeType="application/vnd.google-apps.document"
    )

    doc_file = service.files().create(body=file_metadata, fields="id").execute()
...
def append_transalation(term, translation, doc_id, creds):
    ...
    requests = [
        {
            "insertText": {
                "location": {"index": last_content_item.get("startIndex"),},
...
if __name__ == "__main__":
    ...
    doc_id = create_doc("Test from python server", folder_id, creds)
    for i in range(10):
